...
- **`examples/v0.5-walkthrough.md`** added as a worked example of the full v0.5 procedure end-to-end on a real codebase. Shows in-session-author detection triggering, the existing-docs scan, the doc plan with locked visual vocabulary, the token-budget output, one diagram-procedure invocation including the new "Vocabulary conformance" plan item, the reader-test subagent JSON output, and the assembled generation-notes block. Companion to the existing `examples/walkthrough.md` (which covers the inner diagram procedure only).
